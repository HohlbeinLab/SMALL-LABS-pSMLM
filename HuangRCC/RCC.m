...
%first and last pseudo-intervals determined to be linear interpolation
%of the two closest points. Original RCC assumed slope=0 at this point.
firstpseudointerval = -1*(drift(1,:)/(indexinterp(3)-indexinterp(1)))*indexinterp(2);
lastpseudointerval = drift(end,:)+((drift(end,:)-drift(end-1,:))/(indexinterp(end-1)-indexinterp(end-2)))*(indexinterp(end)-indexinterp(end-1));

finaldrift(:,1) = interp1(indexinterp,[firstpseudointerval(:,1) 0 drift(:,1)' drift(nbinframe-1,1)],1:ntotalframe,'spline');
finaldrift(:,2) = interp1(indexinterp,[firstpseudointerval(:,2) 0 drift(:,2)' drift(nbinframe-1,2)],1:ntotalframe,'spline');
% 
...
% figure(3);clf(3);
% hold on
% scatter(indexinterp,[firstpseudointerval(:,1) 0 drift(:,1)' lastpseudointerval(:,1)])
% scatter(indexinterp,[firstpseudointerval(:,2) 0 drift(:,2)' lastpseudointerval(:,2)])
% keyboard
%% drift interpolation KM
% finaldrift(:,1) = interp1(indexinterp,[drift(:,1)' drift(nbinframe-1,1)],1:ntotalframe,'spline');
% finaldrift(:,2) = interp1(indexinterp,[drift(:,2)' drift(nbinframe-1,2)],1:ntotalframe,'spline');
% indexinterp = linspace(segpara,segpara*(nbinframe-1)-1,size(drift,1));
% finaldrift(:,1) = interp1(indexinterp,[drift(:,1)'],1:ntotalframe,'spline');
% finaldrift(:,2) = interp1(indexinterp,[drift(:,2)'],1:ntotalframe,'spline');
% figure(5);clf(5);scatter(indexinterp,drift(:,1))
% axis([0 20000 -20 0])
% axis([0 20000 -20 0])
